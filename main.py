...
# Smart logging system with batching and filtering
bot.log_queue = []
bot.last_log_summary = {}

async def log_to_channel(message, level="INFO"):
    """Smart logging with batching and filtering"""
    if not LOG_CHANNEL_ID:
        return
    
    # Filter out debug and routine messages unless they're important
    if level == "DEBUG":
        return  # Skip debug messages entirely
    
    # Skip routine INFO messages that spam the channel
    spam_keywords = ["Queue Check:", "Queue Status:", "Queue Results:", "Queue Updated:", "Dashboard Update"]
    if level == "INFO" and any(keyword in message for keyword in spam_keywords):
        return
    ...
    try:
        ...
        bot.log_queue.append(log_entry)
        
        # Send immediately for critical messages
        if level in ["ERROR", "SUCCESS"]:
            await send_log_immediately(log_entry)
        elif len(bot.log_queue) >= 5:  # Batch send when queue is full
            await send_batched_logs()
            
    except Exception as e:
        print(f"Failed to queue log: {e}")

async def send_log_immediately(log_entry):
    ...
    try:
        ...
        if log_channel:
            ...
            embed = discord.Embed(
                title=f"🤖 {log_entry['level']}",
                description=f"**{log_entry['timestamp'].strftime('%H:%M:%S')}**\n{log_entry['message']}",
                color=colors.get(log_entry['level'], 0x00ffcc)
            )
            
            await log_channel.send(embed=embed)
    except Exception as e:
        print(f"Failed to send immediate log: {e}")

async def send_batched_logs():
    """Send multiple logs in a single message"""
    if not bot.log_queue:
        return
    ...
    try:
        ...
        if log_channel:
            # Group logs by level
            log_groups = {"INFO": [], "WARNING": [], "ERROR": [], "SUCCESS": []}
            
            for log_entry in bot.log_queue:
                if log_entry['level'] in log_groups:
                    log_groups[log_entry['level']].append(log_entry)
            
            # Create summary embed
            embed = discord.Embed(
                title="📊 Bot Activity Summary",
                description=f"**Activity from {bot.log_queue[0]['timestamp'].strftime('%H:%M')} - {bot.log_queue[-1]['timestamp'].strftime('%H:%M')}**",
                color=0x00ffcc
            )
            
            # Add fields for each log level
            for level, logs in log_groups.items():
                if logs:
                    emoji = {"INFO": "ℹ️", "WARNING": "⚠️", "ERROR": "❌", "SUCCESS": "✅"}
                    log_text = "\n".join([f"• {log['message'][:100]}..." if len(log['message']) > 100 else f"• {log['message']}" for log in logs[-5:]])  # Show last 5 of each type
                    
                    embed.add_field(
                        name=f"{emoji[level]} {level} ({len(logs)})",
                        value=log_text if log_text else "No messages",
                        inline=False
                    )
        ...
        print(f"Failed to send batched logs: {e}")
        bot.log_queue = []  # Clear queue on error to prevent buildup
...
async def send_log_summary():
    """Send batched logs every 5 minutes"""
    if bot.log_queue:
        await send_batched_logs()
...
async def update_dashboards():
    ...
    try:
        ...
        # Only log significant queue changes (not every minor update)
        queue = load_queue()
        current_queue_state = {}
        for player in queue:
            if player["status"] == "searching":
        ...
        # Only log if there's a significant change (more than 2 player difference)
        if current_queue_state != bot.last_queue_state:
            significant_change = False
            for format_type in ["1v1", "2v2", "3v3"]:
                old_count = bot.last_queue_state.get(format_type, 0)
                new_count = current_queue_state.get(format_type, 0)
            ...
            if significant_change:
                ...
                await log_to_channel(f"📊 Queue Activity: {queue_info if queue_info else 'Empty'}", "INFO")
...
async def queue_checker():
    """Check queue for matches every 15 seconds with smart logging"""
    try:
        queue = load_queue()
        ...
        # Send detailed summary only when matches are created
        if matches_created > 0:
            match_summary = "\n".join([f"• **{m['format']} {m['mode']}** ({m['id'][:8]}): {m['orange']} vs {m['blue']}" for m in match_details])
            await log_to_channel(f"🎮 {matches_created} Matches Created:\n{match_summary}", "SUCCESS")
        
        if failed_matches > 0:
            await log_to_channel(f"⚠️ {failed_matches} matches failed (player timeout)", "WARNING")
        ...
        if timeout_players:
            ...
            save_queue(queue)
            await log_to_channel(f"⏰ Removed {len(timeout_players)} players from queue (10min timeout)", "WARNING")
...
async def update_all_dashboards():
    ...
    try:
        ...
        # Only log if we updated multiple dashboards
        if updated_count > 1:
            await log_to_channel(f"📊 Updated {updated_count} dashboards across server", "INFO")
    except Exception as e:
